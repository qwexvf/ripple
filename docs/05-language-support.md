...
> **As-built honesty (see [`09`](09-review-and-corrections.md) round 2).** The promise is *fully* true for **Tier 0–1** (symbols + git overlay + imports): a new language is a folder + `.scm` + a registry line and gets same-day value. Per-language behaviors that used to leak into `parse` — `is_exported`, method name-qualification — are now `LanguageAdapter` methods, so the seam holds. **Tier 2** (call resolution) and **Tier 3** (cross-service) are *per-language/framework by nature* — they're a small implementation per language, not free `.scm` data — because concepts like Absinthe fields or Ecto schemas are framework-specific. That's a deliberate boundary, not a gap.
